...
class WebhookService {
  ...
  ResponseEntity<Void> webhooks(String source, CloudEvent cdevent, String ceDataJsonString,  HttpHeaders headers) {
    return AuthenticatedRequest.allowAnonymous( {
      echoService.webhooks(source, cdevent, ceDataJsonString, headers.get("Ce-Id").get(0), headers.get("Ce-Specversion").get(0), headers.get("Ce-Type").get(0), headers.get("Ce-Source").get(0))
    })
  }
